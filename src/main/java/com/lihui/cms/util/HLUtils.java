...
public class HLUtils {
	...
	public static PageInfo<?> findByHighLight(ElasticsearchTemplate elasticsearchTemplate, Class<?> clazz, Integer page,
		...
		//当总页数小于或等于导航页码数时
        if (pagesss <= navigatePages) {
            navigatepageNums = new int[pagesss];
		...
		pi.setNavigatepageNums(navigatepageNums);
		return pi;
	}
	
	static int[] navigatepageNums ;
	
	static int pagesss=pi.getPages();
	
	static int pageNum=pi.getPageNum();
	
	static int navigatePages=8;
